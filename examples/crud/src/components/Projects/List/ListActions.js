...
const ProjectListActions = (props) => {
  ...
  const { uiStore, projectStore, onAddProject } = props;

  const selectedIds = () => Array.from(uiStore.projectList.checked.keys());

  const selectAllOnSelectAll = () => {
    uiStore.projectList.selectAll();
  };

  const selectAllOnUnselectAll = () => {
